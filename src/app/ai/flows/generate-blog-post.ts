...
const GenerateBlogPostInputSchema = z.object({
  title: z.string().describe('The title of the blog post, which serves as the primary keyword and topic.'),
});
...
const prompt = ai.definePrompt({
  ...
  Your task is to write a complete, engaging, and informative blog post based on the provided title. You must strictly adhere to the following SEO-first framework:

  **1. Foundation (From Blog Post Title):**
  - The Primary Keyword is derived from the blog post title: \`{{{title}}}\`.
  - Infer the Search Intent (Informational, Commercial, or Transactional) from the title and write the content to match.
