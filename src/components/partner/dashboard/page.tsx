...
export default function PartnerDashboardPage() {
  ...
  const archiveNotification = async (noteId: string) => {
    ...
    useEffect(() => {
      ...
      const staffUnsubscribe = onSnapshot(query(staffRef, where('role', 'in', ['staff', 'admin', 'partner', 'partner_staff'])), (snapshot) => {
                                            ...
                                            <div className="flex-1">
                                                <div className="flex justify-between items-start">
                                                    <p className="text-sm">
                                                        ...
                                                        <Link href={`/partner/orders/${note.orderId}`} className="font-semibold text-primary hover:underline">{note.orderId}</Link>
                                                    </p>
