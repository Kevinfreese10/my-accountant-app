...
export default function PayslipDownloadButton({ payslip, employee, client, currentData, ytdTotals }: PayslipDownloadButtonProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      const taxPaidYtd = ytdTotals?.tax || p.deductions.find(d => d.label === 'Tax')?.amount || 0;
      const grossYtd = ytdTotals?.gross || p.grossPay;
      const contribYtd = ytdTotals?.contrib || p.contributions.reduce((s, i) => s + i.amount, 0);
      const fringeYtd = ytdTotals?.fringe || p.fringeBenefits.reduce((s, i) => s + i.amount, 0);
