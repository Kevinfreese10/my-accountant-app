...
export default function PayslipDownloadButton({ payslip, employee, client, currentData, ytdTotals }: PayslipDownloadButtonProps) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      drawLabelValue('Employed from', employee.joinDate?.toDate ? format(employee.joinDate.toDate(), 'yyyy/MM/dd') : (employee.joinDate ? format(new Date(employee.joinDate), 'yyyy/MM/dd') : 'N/A'), col2X, currentY);
      ...
      autoTable(doc, {
          ...
          body: p.earnings.map((i: PayslipItem) => [i.label, formatCurr(i.amount)]),
          theme: 'plain',
      ...
      autoTable(doc, {
          ...
          body: p.deductions.map((i: PayslipItem) => [i.label, formatCurr(i.amount)]),
          theme: 'plain',
          headStyles: { fontStyle: 'bold', fontSize: 9, textColor: 0, lineWidth: { bottom: 0.1 } },
          columnStyles: { 
              0: { cellWidth: 65, halign: 'left' }, 
      ...
      // --- 6. Secondary Financial Tables (YTD) ---
      ...
      autoTable(doc, {
          ...
          body: p.contributions.map((i: PayslipItem) => [i.label, formatCurr(i.amount)]),
          theme: 'plain',
          headStyles: { fontStyle: 'bold', fontSize: 9, textColor: 0, lineWidth: { bottom: 0.1 } },
          columnStyles: { 
              0: { cellWidth: 65, halign: 'left' }, 
      ...
      const taxPaidYtd = ytdTotals?.tax || p.deductions.find((d: PayslipItem) => d.label === 'Tax')?.amount || 0;
      const grossYtd = ytdTotals?.gross || p.grossPay;
      const contribYtd = ytdTotals?.contributions || p.contributions.reduce((s: number, i: PayslipItem) => s + i.amount, 0);
      const fringeYtd = ytdTotals?.fringe || p.fringeBenefits.reduce((s: number, i: PayslipItem) => s + i.amount, 0);
      ...
      autoTable(doc, {
          ...
          body: [
              ['Taxable earnings', formatCurr(grossYtd)],
              ['Taxable company contributions', formatCurr(contribYtd)],
              ['Taxable fringe benefits', formatCurr(fringeYtd)],
              ['Provision for tax on annual bonus', formatCurr(0)],
              ['Tax paid', formatCurr(taxPaidYtd)],
          ],
          theme: 'plain',
          headStyles: { fontStyle: 'bold', fontSize: 9, textColor: 0, lineWidth: { bottom: 0.1 } },
          columnStyles: { 
      ...
      doc.save(`Payslip-${employee.surname}-${(payslip.period || 'Current').replace(/\s/g, '-')}.pdf`);
