 ...
 */
async function validateWithPayFast(rawBody: string, data: { [key: string]: string }): Promise<boolean> {
    try {
        const isSandbox = 
            process.env.NEXT_PUBLIC_PAYFAST_PROCESS_URL?.includes('sandbox') ||
            process.env.NEXT_PUBLIC_PAYFAST_MERCHANT_ID === '23836312' ||
            data.merchant_id === '23836312' ||
        ...
        console.log(`ITN Validation: Performing postback to ${url} (isSandbox=${isSandbox})`);
...
export async function POST(req: NextRequest) {
    ...
    try {
        ...
        if (isValidSignature) {
            ...
            isAuthentic = await validateWithPayFast(rawBody, data);
            if (isAuthentic) {
